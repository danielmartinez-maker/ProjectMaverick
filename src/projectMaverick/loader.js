(function(root){
  ...
  require(pathPrefix+'ui/ideologyExplorer'); require(pathPrefix+'ui/electoratePanel'); require(pathPrefix+'ui/comparePanel');
  try{
    ...
    const storage=createStorage(root.localStorage,(code,detail)=>diagnostics.record(code,detail));const events=createEvents();const adapter=createAdapter(root);const issueCatalog=adapter.readIssueCatalog();
    const records=adapter.readPoliticians().map(p=>{const scored=scorePolitician(p,issueCatalog);const classified=classifyPolitician(scored);const politicalStyle=classifyPoliticalStyle(p);return{...p,politicalStyle,ideology:{...classified,dimensions:scored.dimensions,evidence:scored.evidence}};});
    ...
    function openDashboard(){shell.open((target)=>renderDashboard(document,target,{records,searchPoliticians,filterPoliticians,watchlists,recentViews,openPolitician}));}
